  ...
  const handleSubmit = () => {
    ...
    for (var i = 0; i < data.length - 1; i++) {
      ...
      for (var j = 0; j <= rankData.length - 1; j++) {
        if (rankData[j].rank1 == data[i].id) {
          a = a + 1;
        }
        if (rankData[j].rank2 == data[i].id) {
          b = b + 1;
        }
        if (rankData[j].rank3 == data[i].id) {
          c = c + 1;
        }
      ...
      <Div>
        ...
        <Desc>
          ...
          <Ul>
            ...
            <Li>
              If there is another dish with the same rank the other dish will
              lose its rank
            </Li>
          </Ul>
        </Desc>
        ...
        <NavLink to="/leaderboard" style={{ textDecoration: "none" }}>
          <Button onClick={handleSubmit}>
            SUBMIT AND GOTO LEADERBOARD <ArrowForwardOutlinedIcon />
          </Button>
        </NavLink>
      </Div>
    </div>
